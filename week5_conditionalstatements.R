...
surveys$hindfoot_cat <- ifelse(surveys$hindfoot_length < mean(surveys$hindfoot_length, na.rm = TRUE),
                               yes = "small", no = "big")
head(surveys)
unique(surveys$hindfoot_cat) # gives the names, categorical data 
...
# case_when doesn't understand NAs 

surveys %>% 
  mutate(hindfoot_cat = case_when(
    hindfoot_length > 31.5 ~ 'big',
    hindfoot_length > 29 ~ 'medium',
    is.na(hindfoot_length) ~ NA_character_, # need to use that for a character column 
    TRUE ~ 'small' # pretty much the catch-all "else" statement 
  )) %>% 
  select(hindfoot_length, hindfoot_cat) %>% 
  head(10)
...
surveys %>%
  mutate(favorites = case_when( # can call any column pretty much 
